...
Agent-facing bodies (`decompressForDisplay`/`previewBody`, `service/httputil.go`): tools that return a body (`flow_get`, send previews, `diff_flow`, `find_reflected`, `js_*`, body search) de-chunk then decompress it; wire headers are left unchanged and `*_size` fields exclude chunk framing (still compressed). A supported `Content-Encoding` that can't be decoded (truncated at `max_body_bytes`, or corrupt) previews as `<COMPRESSED-UNDECODABLE: …>`. Body rules decode transparently (decompress→apply→recompress); multi-value/unknown encodings and over-`max_body_bytes` bodies skip body rules.
