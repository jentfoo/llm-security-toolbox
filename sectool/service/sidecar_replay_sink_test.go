...
func TestIsReplayFlow(t *testing.T) {
	t.Parallel()

	assert.False(t, isReplayFlow(&types.Flow{}))
	assert.False(t, isReplayFlow(&types.Flow{Annotations: map[string]any{wire.AnnotationReplay: false}}))
	assert.True(t, isReplayFlow(&types.Flow{Annotations: map[string]any{wire.AnnotationReplay: true}}))
}
