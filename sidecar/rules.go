...
// the bytes. Callers hold no lock; apply takes the read lock.
...
// applyRule applies a single rule. An empty find appends the replacement; a literal
// find replaces all occurrences (case-insensitive for headers); a regex uses ReplaceAll.
